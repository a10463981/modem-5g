...
define Package/modem-5g/description
 5G Modem management interface with web UI (port 8080).
 有房大佬出品 | Includes: modemserver, quectel-CM-M, sendat, tom_modem.
endef
...
define Package/modem-5g/install
	...
	chmod 0755 $(1)/etc/init.d/quectel-CM
	chmod 0755 $(1)/etc/hotplug.d/usb/20-modem_mode
endef

define Package/modem-5g/postinst
#!/bin/sh
chmod +x /etc/init.d/modemserver /etc/init.d/usbmode /etc/init.d/quectel-CM
...
$(eval $(call BuildPackage,modem-5g))
